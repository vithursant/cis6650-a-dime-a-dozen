...
i = 0
while os.path.isfile('results/' + dataset + '_log_' + str(i) + '.csv'):
    i += 1
test_id = i
test_id = str(test_id)

twitter_logger = CSVLogger(filename='results/' + dataset + '_log_' + test_id + '.csv', fieldnames=['epochs', 'train_cost', 'val_cost', 'best_cost'])
    ...
    with tf.Session() as sess:
        ...
        for i in range(max_epoch):
            ...
            if i % val_every == 0:
                ...
                #best_cost = min(best_cost, vl_cost)
