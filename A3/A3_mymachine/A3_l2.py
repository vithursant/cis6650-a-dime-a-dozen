...
if not os.path.exists('A3_l2'):
    os.makedirs('A3_l2')

i = 0
while os.path.isfile('A3_l2/' + dataset + '_log_' + str(i) + '.csv'):
    i += 1
test_id = i
test_id = str(test_id)

twitter_logger = CSVLogger(filename='A3_l2/' + dataset + '_log_' + test_id + '.csv', fieldnames=['epochs', 'train_cost', 'val_cost', 'best_cost'])
